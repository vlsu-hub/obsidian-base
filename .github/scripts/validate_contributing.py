...
def check_file(filepath):
    ...
    if not filename.startswith('_'):
        ...
        # Регулярка требует ровно один пробел, 2 цифры, пробел, дефис, пробел
        name_regex = rf'^({prefix_pattern}) \d{{2}} - .+\.md$'
        if not re.match(name_regex, filename):
            errors.append(f"Имя файла '{filename}': Нарушен стандарт. Должно быть '[ПРЕФИКС] [ДВЕ ЦИФРЫ] - [Тема].md'.")
